  ...
  submitMovie() {
    fetch(`http://localhost:8081/recommendations/${this.state.movieName}`, {
      ...
      .then((recommendationList) => {
        console.log(recommendationList); //displays your JSON object in the console
        let RecommendationsRes = recommendationList.map((movie, i) => (
          <RecommendationsRow key={i} movie={movie} />
        ));

        //This saves our HTML representation of the data into the state, which we can call in our render function
        this.setState({
          recMovies: RecommendationsRes,
        });
      })
